...
const allAnimalsInEuropeWeighLessThanOnehundred = animals.every((animal) => {
  animal.continents.includes("Europe") && animal.weight < 100;
});
...
const averageWeightOfAllAnimalsInAfrica = animals.filter((animal) => {
  return animal.continents.includes("Africa");
}).map((animal) => {
  return animal.weight;
}).reduce((num1, num2) => num1 + num2) / animals.filter((animal) => {
  return animal.continents.includes("Africa");
}).length;
